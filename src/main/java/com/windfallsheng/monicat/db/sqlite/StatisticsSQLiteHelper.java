package com.windfallsheng.monicat.db.sqlite;
...
public class StatisticsSQLiteHelper extends SQLiteOpenHelper {
    ...
    public static final String COLUMN_TRIGGERING_TIME = "triggering_time";
    public static final String COLUMN_SESSION_TYPE = "session_type";
    public static final String COLUMN_UPLOADE_STATUS = "uploade_status";
    /**
     * 页面停留时间本地数据库表
     */
    ...
    public void onCreate(SQLiteDatabase db) {
         ...
         *
         */
        db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_APP_SESSION
                + "(" + COLUMN_SESSION_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " + COLUMN_DEVICE_INFO_ID + " INTEGER, "
                + COLUMN_TRIGGERING_TIME + " INTEGER, " + COLUMN_SESSION_TYPE + " INTEGER, "
                + COLUMN_UPLOADE_STATUS + " INTEGER)");
        /**
         * user_id          用户ID
         * creat_time       页面打开时间
         * destory_time     页面关闭时间
                ...
                switch (j) {
                    case 2:
                        ...
                        String sqlRename = "ALTER TABLE "
                                + TABLE_APP_SESSION + " RENAME to " + TABLE_APP_SESSION + "_temp";
                        db.execSQL(sqlRename);

                        //2. 创建新表
                        String sqlCreatNew = "CREATE TABLE IF NOT EXISTS " + TABLE_APP_SESSION
                                + "(" + COLUMN_SESSION_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " + COLUMN_DEVICE_INFO_ID + " INTEGER, "
                                + COLUMN_TRIGGERING_TIME + " INTEGER, " + COLUMN_SESSION_TYPE + " INTEGER, "
                                + COLUMN_UPLOADE_STATUS + " INTEGER, added_column INTEGER)";
                        db.execSQL(sqlCreatNew);

                        //3. 导入数据　
                        String sqlInsertInto = "INSERT INTO " + TABLE_APP_SESSION + " SELECT *, '' FROM " + TABLE_APP_SESSION + "_temp";
                        db.execSQL(sqlInsertInto);
                        // 或者
                        // db.execSQL("INSERT INTO " + TABLE_APP_SESSION + "()SELECT * , '' FROM " + TABLE_APP_SESSION+ "_temp");
                        //  注意 双引号”” 是用来补充原来不存在的数据的

                        //4. 删除临时表　　
                        String sqlDropTemp = "DROP TABLE IF EXISTS " + TABLE_APP_SESSION + "_temp";
...
//            Cursor c = db.rawQuery("SELECT * FROM " + TABLE_APP_SESSION + " WHERE 0", null);
//            try {
...
//            }
            ...
            e.printStackTrace();
            Log.d(MonicatConstants.SDK_NAME, "Monicat:SQLiteDatabase upgrade failed.");
        } finally {
            db.endTransaction();
        }
    }
            ...
            for (int j = oldVersion; j >= newVersion; j--) {
                switch (j) {
                    case 1:
                        ...
                        //2. 创建新表
                        db.execSQL(sqlCreatNew);
                        String sqlInsertInto = "INSERT INTO " + TABLE_APP_SESSION + " SELECT " + COLUMN_SESSION_ID + ", " + COLUMN_DEVICE_INFO_ID + ", "
                                + COLUMN_TRIGGERING_TIME + ", " + COLUMN_SESSION_TYPE + ", " + COLUMN_UPLOADE_STATUS
                                + " FROM " + TABLE_APP_SESSION + "_temp";
                        //3. 导入数据　
                        db.execSQL(sqlInsertInto);
                        // 或者
                        // db.execSQL("INSERT INTO " + TABLE_APP_SESSION + "()SELECT * , '' FROM " + TABLE_APP_SESSION + "_temp");
...
//            }
            ...
            e.printStackTrace();
            Log.d(MonicatConstants.SDK_NAME, "Monicat:SQLiteDatabase downgrade failed.");
        } finally {
            db.endTransaction();
        }
    }
