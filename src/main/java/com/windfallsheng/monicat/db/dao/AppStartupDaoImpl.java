 ...
 * Version:
 */
public class AppStartupDaoImpl implements IBaseDao<AppStartupEntity> {

    private volatile static AppStartupDaoImpl instance = null;
    private static StatisticsSQLiteHelper sStatisticsSQLiteHelper;

    private AppStartupDaoImpl() {
    }
    ...
    public static IBaseDao getInstance(Context context) {
        ...
        if (instance == null) {
            synchronized (AppStartupDaoImpl.class) {
                if (instance == null) {
                    instance = new AppStartupDaoImpl();
                }
            }
        }
        return instance;
    ...
    @Override
    public int save(AppStartupEntity appStartupEntity) {
        SQLiteDatabase db = sStatisticsSQLiteHelper.getDb();
        db.beginTransaction();
        try {
            ContentValues values = new ContentValues();
            values.put(StatisticsSQLiteHelper.COLUMN_DEVICE_INFO_ID, appStartupEntity.getDeviceInfoId());
            values.put(StatisticsSQLiteHelper.COLUMN_STARTUP_TIME, appStartupEntity.getStartupTime());
            values.put(StatisticsSQLiteHelper.COLUMN_STARTUP_TYPE, appStartupEntity.getStartupType());
            values.put(StatisticsSQLiteHelper.COLUMN_HAS_UPLOADED, appStartupEntity.getHasUploaded());
            LogUtils.d(Constants.SDK_NAME, "AppStartupDaoImpl_save_values==" + values.toString());
            db.insert(StatisticsSQLiteHelper.TABLE_APP_STARTUP, "id", values);
            StringBuffer sbSQL = new StringBuffer();
            sbSQL.append("SELECT LAST_INSERT_ROWID() From ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP);
//            sbSQL.append("SELECT MAX(").append(StatisticsSQLiteHelper.COLUMN_STARTUP_ID).append(") FROM ")
//                    .append(StatisticsSQLiteHelper.TABLE_APP_STARTUP);
            String sql = sbSQL.toString();
            LogUtils.d(Constants.SDK_NAME, "StartupNumDbHelper_last_insert_rowid()_sql==" + sql);
            Cursor cursor = db.rawQuery(sql, null);
            int primaryKeyId = 0;
            if (cursor.moveToFirst()) {
                primaryKeyId = cursor.getInt(0);
                LogUtils.d(Constants.SDK_NAME, "AppStartupDaoImpl_save_last_insert_rowid==" + primaryKeyId);
            }
    ...
    public List<AppStartupEntity> queryAllByMap(ParamMap conditions) {
        List<AppStartupEntity> startupNumEntities = new ArrayList<>();
        SQLiteDatabase db = sStatisticsSQLiteHelper.getDb();
        StringBuffer sbSQL = new StringBuffer();
        try {
            sbSQL.append("SELECT * FROM ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP)
                    .append(" WHERE 1 = 1 ");
//            sbSQL.append("SELECT ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_ID).append(", ").append(StatisticsSQLiteHelper.COLUMN_DEVICE_INFO_ID)
//                    .append(", ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_TIME).append(", ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_TYPE)
//                    .append(", ").append(StatisticsSQLiteHelper.COLUMN_HAS_UPLOADED).append(" FROM ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP)
//                    .append(" WHERE 1 = 1 ");
            ...
            LogUtils.d(Constants.SDK_NAME, "AppStartupDaoImpl-->queryAllByMap()_sql==" + sql);
            Cursor cursor = db.rawQuery(sql, null);
            while (cursor.moveToNext()) {
                int index = 0;
                AppStartupEntity startupNumEntity = new AppStartupEntity();
                startupNumEntity.setStartupId(cursor.getInt(index++));
                startupNumEntity.setDeviceInfoId(cursor.getInt(index++));
                startupNumEntity.setStartupTime(cursor.getLong(index++));
                startupNumEntity.setStartupType(cursor.getInt(index++));
                startupNumEntity.setHasUploaded(cursor.getInt(index++));
     ...
     */
    @Override
    public AppStartupEntity queryAdjacentData(Serializable id, ParamMap conditions, int lastOrNext) {
...
//        2.select * from table_a where id = (select min(id) from table_a where id > {$id});
        AppStartupEntity startupNumEntity = null;
        SQLiteDatabase db = sStatisticsSQLiteHelper.getDb();
        StringBuffer sbSQL = new StringBuffer();
        try {
            sbSQL.append("SELECT ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_ID).append(", ").append(StatisticsSQLiteHelper.COLUMN_DEVICE_INFO_ID)
                    .append(", ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_TIME)
                    .append(", ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_TYPE).append(", ").append(StatisticsSQLiteHelper.COLUMN_HAS_UPLOADED)
                    .append(" FROM ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP).append(" WHERE ")
                    .append(StatisticsSQLiteHelper.COLUMN_STARTUP_ID).append(" = ( SELECT ");
            if (lastOrNext == Constants.QUERY_DATA_LAST) {
                sbSQL.append(" min( ");
            } else if (lastOrNext == Constants.QUERY_DATA_NEXT) {
                sbSQL.append(" max( ");
            }
            sbSQL.append(StatisticsSQLiteHelper.COLUMN_STARTUP_ID).append(" ) FROM ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP)
                    .append(" WHERE 1 = 1 ");
            if (conditions != null) { // 拼接条件进行查询的语句
                String spliceSQL = SQLUtils.spliceSQL(conditions);
                sbSQL.append(spliceSQL);
            ...
            if ((int) id > 0) {
                sbSQL.append(" AND ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_ID);
                if (lastOrNext == Constants.QUERY_DATA_LAST) {
                    sbSQL.append(" > ");
                } else if (lastOrNext == Constants.QUERY_DATA_NEXT) {
                    sbSQL.append(" < ");
            ...
            String sql = sbSQL.toString();
            LogUtils.d(Constants.SDK_NAME, "AppStartupDaoImpl-->queryAdjacentData()_sql==" + sql);
            Cursor cursor = db.rawQuery(sql, null);
            while (cursor.moveToNext()) {
                int index = 0;
                startupNumEntity = new AppStartupEntity();
                startupNumEntity.setStartupId(cursor.getInt(index++));
                startupNumEntity.setDeviceInfoId(cursor.getInt(index++));
                startupNumEntity.setStartupTime(cursor.getLong(index++));
                startupNumEntity.setStartupType(cursor.getInt(index++));
            ...
            LogUtils.d(Constants.SDK_NAME, "AppStartupDaoImpl-->queryAdjacentData()_e == " + e.toString());
            e.printStackTrace();
        } finally {
            sStatisticsSQLiteHelper.closeSQLiteDatabase();
        }
    ...
    public AppStartupEntity queryById(Serializable id) {
        ...
        try {
            sbSQL.append("SELECT ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_ID).append(", ").append(StatisticsSQLiteHelper.COLUMN_DEVICE_INFO_ID)
                    .append(", ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_TIME).append(", ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_TYPE)
                    .append(", ").append(StatisticsSQLiteHelper.COLUMN_HAS_UPLOADED).append(" FROM ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP)
                    .append(" WHERE ").append(StatisticsSQLiteHelper.COLUMN_STARTUP_ID).append(" = ").append(id);
            String sql = sbSQL.toString();
//            LogUtils.d(Constants.SDK_NAME, "StartupNumDbHelper_queryById()_sql==" + sql);
    ...
    public int queryCountByMap(ParamMap conditions) {
        ...
        StringBuffer sbSQL = new StringBuffer();
        try {
            sbSQL.append("SELECT  COUNT(*) FROM ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP).append(" WHERE 1 = 1 ");
            if (conditions != null) { // 拼接条件进行查询的语句
                String spliceSQL = SQLUtils.spliceSQL(conditions);
                sbSQL.append(spliceSQL);
            }
            String sql = sbSQL.toString();
            LogUtils.d(Constants.SDK_NAME, "AppStartupDaoImpl-->queryCountByMap()_sql==" + sql);
            Cursor cursor = db.rawQuery(sql, null);
            while (cursor.moveToNext()) {
    ...
    public void updateByMap(ParamMap conditions) {
        ...
        try {
            if (conditions != null) {
                StringBuffer sbSQL = new StringBuffer();
                sbSQL.append("UPDATE ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP).append(" SET ");
                ...
                LogUtils.d(Constants.SDK_NAME, "AppStartupDaoImpl-->updateByMap()_sql==" + sql);
                db.execSQL(sql);
                db.setTransactionSuccessful();
            } else {
...
//                primaryKeyId = db.update(StatisticsSQLiteHelper.TABLE_APP_STARTUP, values, whereClause, whereArgs);
//                db.setTransactionSuccessful();
...
//        return primaryKeyId;
    ...
    public void deleteByMap(ParamMap conditions) {
        ...
        StringBuffer sbSQL = new StringBuffer();
        try {
            sbSQL.append("DELETE FROM ").append(StatisticsSQLiteHelper.TABLE_APP_STARTUP)
                    .append(" WHERE 1 = 1 ");
            if (conditions != null) { // 拼接条件进行查询的语句
            ...
            // OR startup_type = 1
            LogUtils.d(Constants.SDK_NAME, "AppStartupDaoImpl-->deleteByMap()_sql==" + sql);
            db.execSQL(sql);
            db.setTransactionSuccessful();
        } catch (Exception e) {
            e.printStackTrace();
