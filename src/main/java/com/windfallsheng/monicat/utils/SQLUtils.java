...
public class SQLUtils {
    ...
    public static String spliceQueryParamMap(ParamMap conditions) {
        ...
        StringBuffer sbSQL = new StringBuffer();
        if (andList != null) {
            String andSQL = SQLUtils.spliceSQLWhere(andList, Constants.AND);
            sbSQL.append(andSQL);
        }
        if (orList != null) {
            String orSQL = SQLUtils.spliceSQLWhere(orList, Constants.OR);
            sbSQL.append(orSQL);
        }
        if (andLikeList != null) {
            String andLikeSQL = SQLUtils.spliceSQLWhere(andLikeList, Constants.AND_LIKE);
    ...
    public static String spliceInParamMap(ParamMap conditions) {
        ...
        String andInSQL = "";
        if (andInMap != null) {
            andInSQL = SQLUtils.spliceSQLIn(andInMap, Constants.AND);
        }
        return andInSQL;
    }
    ...
    private static String spliceSQLIn(Map<String, List<Object>> params, int flag) {
        ...
        if (params != null && params.size() > 0) {
            for (String key : params.keySet()) {
                ...
                if (flag == Constants.AND) {
                    sbSQL.append(" AND ");
                } else if (flag == Constants.OR) {
                    sbSQL.append(" OR ");
                }
    ...
    public static String spliceOrderParamMap(ParamMap conditions) {
        ...
        if (orderDescList != null) {
            sbSQL = SQLUtils.spliceSQLOrder(orderDescList, Constants.ORDER_BY_DESC);
        } else if (orderAscList != null) {
            sbSQL = SQLUtils.spliceSQLOrder(orderAscList, Constants.ORDER_BY_ASC);
        }
        return sbSQL;
    }
        ...
        if (params != null && params.size() > 0) {
            for (Param param : params) {
                ...
                if (flag == Constants.AND) {
                    ...
                    sbSQL.append(" OR ");
                } else if (flag == Constants.AND_LIKE || flag == Constants.AND_IN) {
                    sbSQL.append(key + " AND ");
                }
                if (flag == Constants.AND || flag == Constants.OR) {
                    sbSQL.append(key + " = ");
                    if (value instanceof String) {
                        sbSQL.append("'").append(value).append("'");
                    } else {
                ...
                } else if (flag == Constants.AND_LIKE) {
                    sbSQL.append(key + " LIKE ");
                    sbSQL.append("'%").append(value).append("%'");
                } else if (flag == Constants.AND_IN) {
                    sbSQL.append(key + " IN( ");
    ...
    private static String spliceSQLOrder(List<String> params, int flag) {
        ...
        if (flag == Constants.ORDER_BY_DESC) {
            sbSQL.append(" DESC ");
        } else if (flag == Constants.ORDER_BY_ASC) {
            sbSQL.append(" ASC ");
        }
        return sbSQL.toString();
    }
